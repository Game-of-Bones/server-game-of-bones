...
// Importamos la clase Sequelize de sequelize-typescript
import { Sequelize } from 'sequelize-typescript'; 
import dotenv from 'dotenv';
import path from 'path'; // Necesario para resolver la ruta de los modelos
...
const sequelize = new Sequelize({
    ...
    // --- NUEVA CONFIGURACIÓN PARA DECORADORES ---
    // Especificamos la ruta a los modelos. 
    // Usamos 'glob' para encontrar todos los archivos .ts en la carpeta 'models'
    // que terminan en .ts (excluyendo archivos de mapeo .d.ts)
    models: [path.join(__dirname, '../models/*.ts')], 
    // Si usaras archivos .js o .d.ts generados por el build, podrías usar:
